...
// TestComposeSingleError makes sure that composing an error with nil returns
// the original error instead of [originalErr].
func TestComposeSingleError(t *testing.T) {
	err := New(t.Name())
	composedErr := Compose(err, nil)
	if composedErr != err {
		t.Fatal("composed error should match err")
	}
	if !Contains(composedErr, err) {
		t.Fatal("composed error should contain err")
	}
}
